...
# Use load() instead of preload() to gracefully handle missing textures in headless mode
var non_hit_impact_scn: PackedScene = null

func _init() -> void:
	if ResourceLoader.exists("res://assets/effects/impact_spark.tscn"):
		non_hit_impact_scn = load("res://assets/effects/impact_spark.tscn")
...
func show_non_hit_imact() -> void:
	if non_hit_impact_scn == null:
		return  # Gracefully skip in headless mode when textures unavailable
