...
static func cardinal_extrapolate_3d(pts: Array[Vector3]) -> void:
    if pts.size() == 0:
        print_debug("cardinal_extrapolate: Cant extrapolate empty array")
        return
    if pts.size() == 1:
        pts.push_back(pts[0])
        pts.push_back(pts[0])
...
static func curve3d_cardinal_extrapolate(curve: Curve3D) -> void:
    if curve.point_count == 0:
        print_debug("curve3d_cardinal_extrapolate: Cant extrapolate empty curve")
        return
    if curve.point_count == 1:
...
static func cardinal_extrapolate_2d_discontinuous(pts: Array[Vector2]) -> void:
    if pts.size() == 0:
        print_debug("cardinal extrapolate 2d disc: Cant extrapolate empty array")
        return
    if pts.size() % 2 != 0:
...
static func curve2d_cardinal_extrapolate(curve: Curve2D) -> void:
    if curve.point_count == 0:
        print_debug("curve2d_cardinal_extrapolate: Cant extrapolate empty curve")
        return
    if curve.point_count == 1:
